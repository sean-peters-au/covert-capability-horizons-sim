...
from .gates import delta50_precision_gate, delta50_in_range_gate, trend_recovery_rope_gate
...
def cmd_simulate(args: argparse.Namespace) -> None:
    ...
    attempts = simulate_model_attempts(
        ...
        tasks=tasks, models=models_list,
        sc_alpha=cfg.sc_alpha, sc_beta=cfg.sc_beta, attempts_per_pair=cfg.attempts_per_pair,
    )
    mon0 = cfg.monitors[0]
    models_mon = apply_detection_models(attempts, dict(
        id=mon0.id,
    ...
    for mon, d in trend.get("trend", {}).items():
        ...
        rows_tr.append(dict(monitor_id=mon,
                            slope_median=d.get("slope_median"), slope_lo=slo_lo, slope_hi=slo_hi,
                            doubling_months_median=d.get("dm_median"), doubling_months_lo=dm_lo, doubling_months_hi=dm_hi))
    ...
    for key, arr in mod_draws.get("delta50_s_draws", {}).items():
        mon_id, mid = key.split(":"); d50_med = np.percentile(np.array(arr, float), 50.0)
        rel = abs(Delta_task_med - d50_med) <= 0.3 * max(d50_med, 1e-6)
        absw = abs(Delta_task_med - d50_med) <= 30.0
        cov_rows.append(dict(monitor_id=mon_id, model_id=mid, release_month=rel_map.get(mid),
                             near_rel_count=int(np.sum(rel)), near_abs30s_count=int(np.sum(absw)),
    ...
    for tid, g in humans.groupby("task_id"):
        ...
        nT = int(T.size); nTC = int(TC.size)
        vT = float(T.var(ddof=1)) if nT > 1 else float('nan')
        vTC = float(TC.var(ddof=1)) if nTC > 1 else float('nan')
        se = float(np.sqrt(max(vT,0.0)/max(nT,1) + max(vTC,0.0)/max(nTC,1))) if np.isfinite(vT) and np.isfinite(vTC) else float('nan')
    ...
    # Minimal summary
    (Path(args.out) / "summary.json").write_text(json.dumps({"n_tasks": len(tasks), "n_models": len(models_list)}, indent=2))
...
def cmd_sweep(args: argparse.Namespace) -> None:
    ...
    true_dm = float(gates.get("delta50_true_doubling_months", gates.get("true_doubling_months", float("nan"))))
    ...
    for sc_idx, sc in enumerate(scenarios, start=1):
        ...
        for r in range(1, replicates + 1):
            cfg = _cfg_from_dict(sc.get("cfg", {})); cfg.seed = int(1000*sc_idx + r); cfg.validate()
            tasks = generate_tasks(seed=cfg.seed, n_t_bins=cfg.n_t_bins, tasks_per_bin=cfg.tasks_per_bin,
                                   t_seconds_min=cfg.t_seconds_min, t_seconds_max=cfg.t_seconds_max,
                                   sigma_task=cfg.sigma_task, c_over_bins=cfg.c_over_bins or [],
                                   c_over_mix_by_t_bin=cfg.c_over_mix_by_t_bin or [], c_over_sample=cfg.c_over_sample)
            humans = simulate_humans(seed=cfg.seed, tasks=tasks, n_participants=cfg.n_participants,
                                     repeats_per_condition=cfg.repeats_per_condition, sigma_participant=cfg.sigma_participant,
                                     sigma_noise=cfg.sigma_noise, human_skill_sd=cfg.human_skill_sd,
                                     human_cov_alpha=cfg.human_cov_alpha, human_cov_beta=cfg.human_cov_beta)
            ...
            models_list = generate_models(cfg.__dict__, np.random.default_rng(cfg.seed))
            attempts = simulate_model_attempts(rng=np.random.default_rng(cfg.seed), tasks=tasks, models=models_list,
                                               sc_alpha=cfg.sc_alpha, sc_beta=cfg.sc_beta, attempts_per_pair=cfg.attempts_per_pair)
            rel_map = {m["model_id"]: int(m.get("release_month", 0)) for m in models_list}
            for mon in cfg.monitors:
                mon_d = dict(id=mon.id, fpr_target=mon.fpr_target, fpr_short=mon.fpr_short, fpr_med=mon.fpr_med, fpr_long=mon.fpr_long, det_b1=mon.det_b1, eta=mon.eta)
                models_mon = apply_detection_models(attempts, mon_d); models_mon["monitor_id"] = mon.id
                ...
                for key, arr in draws_m.get("delta50_s_draws", {}).items():
                    mon_id, mid = key.split(":"); lo, med, hi = np.percentile(np.array(arr, float), [2.5, 50.0, 97.5])
                    rows.append(dict(scenario=name, seed=cfg.seed, monitor_id=mon_id, model_id=mid,
                                     release_month=rel_map.get(mid), delta50_s=med, delta50_s_lo=lo, delta50_s_med=med, delta50_s_hi=hi))
                tr = sample_trend_posterior(draws_m, rel_map, priors={"seed": cfg.seed, "num_warmup_trend": 200, "num_samples_trend": 200})
                d = tr.get("trend", {}).get(mon.id, None)
                if d:
                    slo_lo, slo_hi = (d.get("slope_ci") or [float("nan"), float("nan")])
                    dm_lo, dm_hi = (d.get("dm_ci") or [float("nan"), float("nan")])
                    # Posterior gate probability if threshold is provided
                    p_gate = float(np.mean(np.array(d.get("doubling_months_draws", []), float) <= trend_max_dm)) if math.isfinite(trend_max_dm) and d.get("doubling_months_draws") is not None else float("nan")
                    trend_rows.append(dict(scenario=name, seed=cfg.seed, monitor_id=mon.id,
                                           slope=d.get("slope_median"), slope_lo=slo_lo, slope_hi=slo_hi,
                                           doubling_months=d.get("dm_median"), doubling_months_lo=dm_lo, doubling_months_hi=dm_hi,
                ...
                if np.isfinite(delta50_max_ci_width_s):
                    pass_any = False
                    for key, arr in draws_m.get("delta50_s_draws", {}).items():
                        g = delta50_precision_gate(np.asarray(arr, float), max_width_seconds=delta50_max_ci_width_s)
                ...
                if "d_seconds" in models_mon.columns:
                    ...
                    series = None
                dmins = float(series.min()) if series is not None else float("nan"); dmaxs = float(series.max()) if series is not None else float("nan")
                if np.isfinite(dmins) and np.isfinite(dmaxs):
                    pass_any = False
                    for key, arr in draws_m.get("delta50_s_draws", {}).items():
                        g = delta50_in_range_gate(np.asarray(arr, float), dmins, dmaxs, min_fraction_in_range=d50_min_fraction)
                        if bool(g.get("pass_", False)):
                            pass_any = True; break
    ...
    if trend_rows:
        ...
        (Path(args.out) / "gate_params.json").write_text(json.dumps(dict(trend_max_doubling_months=trend_max_dm), indent=2))
...
def cmd_design_search(args: argparse.Namespace) -> None:
    ...
    if not search_cfg:
        raise RuntimeError("No study_search configuration found (provide --search or add study_search to scenario YAML)")
    os.makedirs(args.out, exist_ok=True)
