...
def sample_humans_posterior(humans_df: pd.DataFrame, priors: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    def model(y, tj, pi, zc, cens):
        ...
        betaC = numpyro.sample("betaC", dist.Normal(p.get("betaC_loc", 0.3), p.get("betaC_sd", 0.25)))
...
def sample_models_posterior(attempts_df: pd.DataFrame,
                            humans_draws: Dict[str, Any],
                            priors: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    draws = humans_draws.get("draws")
    tT_s = np.array(humans_draws["tT_s"])      # [n_tasks, S]
    ...
    for (mon, mid), g in df.groupby(["monitor_id", "model_id"]):
        ...
        def logit_model(x, y):
            theta0 = numpyro.sample("theta0", dist.Normal(0.0, priors.get("theta0_sd", 5.0) if priors else 5.0))
            beta1 = numpyro.sample("beta1", dist.Normal(0.0, priors.get("beta1_sd", 2.0) if priors else 2.0))
        ...
        mcmc = MCMC(nuts, num_warmup=int((priors or {}).get("num_warmup", 600)), num_samples=int((priors or {}).get("num_samples", 600)), num_chains=int((priors or {}).get("num_chains", 4)))
        ...
        try:
            ...
            if not np.isfinite(xmin): xmin = None
            if not np.isfinite(xmax): xmax = None
        except Exception:
            xmin = xmax = None
        lo = 0.0
        hi = None
...
def sample_trend_posterior(metric_draws: Dict[str, Any],
    ...
    for mon in set(k.split(":")[0] for k in keys):
        ...
        def frac_year(yyyymm: int) -> float:
            y = int(yyyymm) // 100; m = int(yyyymm) % 100
            return y + (m - 1) / 12.0
        x_year = np.array([frac_year(release_months[k.split(":")[1]]) for k in mon_keys], float)  # [M]
