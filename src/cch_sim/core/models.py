...
def generate_models(cfg: Dict, rng: np.random.Generator) -> List[Dict]:
    ...
    if mode == "trend":
        ...
        for i, x in enumerate(months):
            ...
            out.append(dict(model_id=f"model_{i+1:02d}", release_month=rm, ability_seconds=float(d50)))
        return out
    # fallback
    ...
    *,
    ...
    for m in models:
        for _, trow in tasks.iterrows():
            ...
            d_seconds = float(trow.get("c_overhead_s", np.exp(trow.log_t_baseline_true)*(np.exp(trow.delta_true)-1.0)))
            ...
            for a in range(attempts_per_pair):
                rows.append(dict(
                    model_id=m["model_id"], release_month=int(m["release_month"]),
                    task_id=trow.task_id, attempt_id=a,
                    runtime_s=runtime_s,
                    d_seconds=d_seconds,
                    delta_true=float(trow.delta_true),
                    u_cov=float(rng.random()), u_det=float(rng.random()), p_cov=float(p_cov)
                ))
