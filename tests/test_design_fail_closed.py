...
def test_eval_candidate_seed_fail_closed(monkeypatch):
    ...
    # Models and attempts
    monkeypatch.setattr(
        ds, "generate_models", lambda cfg, rng: [dict(model_id="m0", release_month=202401)]
    )
    monkeypatch.setattr(
        ds,
        "simulate_model_attempts",
        lambda **kw: pd.DataFrame(
            {
                "model_id": ["m0"],
                "task_id": ["T0"],
    ...
    monkeypatch.setattr(
        ds, "apply_detection_models", lambda df, mon: df.assign(monitor_id=mon.get("id", "M0"))
    )
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: (_ for _ in ()).throw(RuntimeError("Stage2 failure")),
    )
