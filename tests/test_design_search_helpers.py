...
def test_minutes_per_run_and_cost_helpers():
    ...
    S = 10
    draws = {"tT_s": np.tile([[600.0], [600.0]], (1, S)), "Delta_s": np.tile([[100.0], [100.0]], (1, S))}
...
def test_design_search_run_with_fake_eval(tmp_path, monkeypatch):
    ...
    fake_results = [({"delta50_precision": True, "delta50_in_range": True, "delta50_trend_rope": True}, 15.0, 3),
                    ({"delta50_precision": False, "delta50_in_range": True, "delta50_trend_rope": True}, 16.0, 3)]
    ...
    cfg = SimConfig(n_t_bins=2, tasks_per_bin=2, n_participants=2, repeats_per_condition=1,
                    t_seconds_min=30, t_seconds_max=120, c_over_bins=[{"lo_s": 1, "hi_s": 2}], c_over_mix_by_t_bin=[[1.0], [1.0]])
    gates = {"delta50_max_ci_width_s": 60.0, "d50_min_fraction_in_range": 0.8, "delta50_true_doubling_months": 6.0}
    search = {"attempts_per_pair": [10], "tasks_per_bin": [2], "n_participants": [2], "repeats_per_condition": [1], "seeds": 2, "base_seed": 0}
