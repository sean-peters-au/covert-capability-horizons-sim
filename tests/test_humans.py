...
def test_pipeline_humans_models_trend_small_smoke():
    ...
    from cch_sim.pipeline import sample_humans_posterior, sample_models_posterior, sample_trend_posterior
    ...
    for tid, base in [("T0", 1.0), ("T1", 1.2)]:
        for pid in participants:
            ...
            rows.append(dict(participant_id=pid, task_id=tid, condition="T", log_t_obs=base + u_part[pid] + eps_T, censored=0))
            rows.append(dict(participant_id=pid, task_id=tid, condition="T+C", log_t_obs=base + deltas[tid] + u_part[pid] + eps_TC, censored=0))
    ...
    tr = sample_trend_posterior(d50_draws, release, priors=dict(seed=0, num_warmup_trend=200, num_samples_trend=200))
    d = tr["trend"]["M0"]
